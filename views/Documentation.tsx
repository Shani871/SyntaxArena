...
export const Documentation: React.FC = () => {
    ...
    const handleAICreateDocument = (title: string, category: string, content: string) => {
        const newDoc: Doc = {
            id: Date.now().toString(),
            title,
            category,
                ...
                {selectedDoc ? (
                    <>
                        ...
                        <div className="h-14 border-b border-[#333] bg-[#252526] flex items-center justify-between px-6 shrink-0">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded text-xs font-bold transition-colors flex items-center gap-2 border ${showAssistant
                                        ? 'bg-cyber-purple/20 text-cyber-purple border-cyber-purple'
                                        : 'bg-[#333] hover:bg-[#444] text-[#ccc] border-transparent'
                                        }`}
                                >
                                    <Sparkles size={14} /> <span className="hidden md:inline">{showAssistant ? 'Hide Assistant' : 'Ask AI'}</span>
                                </button>
                        ...
                        <div className="flex-1 flex overflow-hidden">
                            {/* Editor / View */}
                            <div className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col pb-24">
                            ...
                            {showAssistant && (
                                <div className="w-80 border-l border-[#333] shrink-0 z-10 bg-[#1e1e1e]">
                                    <DocChat
                                        documentContent={isEditing ? (editForm.content || "") : selectedDoc.content}
                                        onClose={() => setShowAssistant(false)}
                                        onCreateDocument={handleAICreateDocument}
                                    />
                                </div>
                            )}
                        </div>
