...
export const CodeEditor: React.FC<CodeEditorProps> = ({ code, setCode, language = 'JavaScript', readOnly = false, preventPaste = false }) => {
    ...
    <div className="w-full h-full bg-[#1e1e1e] flex flex-col overflow-hidden font-mono text-sm relative">
      ...
      <div className="bg-[#252526] flex items-center border-b border-[#2b2b2b] select-none h-9">
        ...
        >
          ...
          <span className="text-[#f1e05a] font-bold">{getExt(language).toUpperCase()}</span>
          <span>solution.{getExt(language)}</span>
        ...
        >
          ...
          <span>tests.spec.{getExt(language)}</span>
        </div>
      </div>
      ...
      <div className="bg-[#2d2d2d] border-t border-[#333] px-3 py-1 text-[11px] text-[#ccc] flex justify-between items-center select-none z-10 h-6">
        ...
        <div className="flex gap-4 opacity-80">
          ...
          <span className="hover:text-white cursor-pointer text-cyber-blue font-bold">{language}</span>
